  ...
  async upload(
    ...
    if (kind === 'PAYMENT_RECEIPT' && file.mimetype.startsWith('image/')) {
      try {
        ...
        const values = [...text.matchAll(/(?:R\$\s*)?(\d{1,6}[.,]\d{2})/g)].map(m => Number(m[1].replace('.', '').replace(',', '.')));
        const expected = Number(order.payments[0]?.amount || order.total), extracted = values.sort((a,b)=>Math.abs(a-expected)-Math.abs(b-expected))[0];
        await this.db.payment.updateMany({ where: { orderId }, data: { receiptExtraction: { text: text.slice(0, 4000), extractedValue: extracted || null, expectedValue: expected, divergent: extracted ? Math.abs(extracted-expected)>0.01 : true }, receiptConfidence: result.data.confidence / 100, ...(extracted && Math.abs(extracted-expected)>0.01 ? { status: 'REVIEW', reviewReason: 'OCR identificou valor divergente no comprovante' } : {}) } });
      } catch { await this.db.payment.updateMany({where:{orderId},data:{receiptExtraction:{error:'OCR não conseguiu processar o arquivo'},receiptConfidence:0}}); }
    }
