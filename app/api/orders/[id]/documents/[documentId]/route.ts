import { getAuthenticatedUser } from '@/app/chatgpt-auth';
import { getD1 } from '@/db';
import { createPrivateDocumentUrl } from '@/modules/documents/storage';
import { getAccountContext } from '@/modules/identity/service';
import { redirect } from 'next/navigation';

export async function GET(
  _request: Request,
  { params }: { params: Promise<{ id: string; documentId: string }> },
) {
  const user = await getAuthenticatedUser();
  if (!user) return Response.json({ error: 'Faça login.' }, { status: 401 });
  const account = await getAccountContext(user);
  if (!account)
    return Response.json({ error: 'Conta necessária.' }, { status: 403 });
  const { id, documentId } = await params;
  const document = await getD1()
    .prepare(
      `SELECT d.storage_key storageKey FROM order_documents d JOIN orders o ON o.id=d.order_id
     WHERE d.id=? AND d.order_id=? AND (?=TRUE OR o.reseller_organization_id=? OR o.supplier_organization_id=?)`,
    )
    .bind(
      documentId,
      id,
      account.organization.type === 'platform',
      account.organization.id,
      account.organization.id,
    )
    .first<{ storageKey: string }>();
  if (!document)
    return Response.json(
      { error: 'Documento não encontrado.' },
      { status: 404 },
    );
  const url = await createPrivateDocumentUrl(document.storageKey);
  redirect(url);
}
