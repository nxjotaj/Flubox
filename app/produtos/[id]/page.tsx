...
export default async function EditProductPage({
  ...
  const [product, variants, media] = await Promise.all([
    getD1()
      .prepare(
    ...
    getD1()
      .prepare(
        `SELECT id,sku,name,gtin,attributes_json attributesJson,price_cents priceCents,suggested_retail_cents retailCents,stock FROM product_variants WHERE product_id=? AND status='active' ORDER BY created_at`,
      )
      .bind(id)
      .all<{
        id: string;
    ...
    getD1()
      .prepare(
        `SELECT id,alt_text altText,sort_order sortOrder FROM product_media WHERE product_id=? ORDER BY sort_order`,
      )
      .bind(id)
      .all<{ id: string; altText: string; sortOrder: number }>(),
  ]);
  if (!product) notFound();
  return (
    <AppShell account={account} activePath="/produtos">
      <section className="page-heading">
        ...
        <a className="secondary-action" href="/produtos">
          Voltar aos produtos
        </a>
      </section>
      <section className="surface-card">
        <ProductMediaManager productId={id} media={media.results} />
        <EditProductForm
          id={id}
          product={product}
          variants={variants.results.map((item) => ({
            ...item,
            attributes: JSON.parse(item.attributesJson || '{}') as Record<
              string,
              string
            >,
          }))}
